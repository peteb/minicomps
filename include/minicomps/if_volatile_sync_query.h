...
public:
  ...
  if_volatile_sync_query(if_volatile_sync_query& other) {
    ...
    auto handling_component_sp = other.handling_component_.lock();
    assert(handling_component_sp);

    linked_handling_component_ = handling_component_sp.get(); // TODO: check for null
